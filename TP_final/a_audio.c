...
int audio_init(void)
{
    ...
    if (!al_install_audio() || !al_init_acodec_addon() ||!al_reserve_samples(NUM_RESERVED_SAMPLES)){
...
void audio_play(game_sound snd){
    ...
    al_play_sample(samples[snd], 1.0, ALLEGRO_AUDIO_PAN_NONE, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
